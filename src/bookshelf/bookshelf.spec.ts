...
describe('Last position', () => {
    test('should be `null` if reading journey is empty', async () => {
        const dracula = new FakeNote('Books/Dracula.md', new StaticMetadata({ pages: 200 }), [])
        await bookshelf.process(dracula)

        const book = bookshelf.book(dracula)

        expect(book.lastPosition).toBe(null)
    })

    test('should return the position of the last progress item', async () => {
        const dracula = new FakeNote('Books/Dracula.md', new StaticMetadata({ pages: 200 }), [
            '2025-01-02: 1-200',
            '2025-01-06: Finished reading',
            '2026-03-10: Started reading',
            '2025-03-10: 1-150',
            '2025-03-10: Abandoned book',
        ])
        await bookshelf.process(dracula)

        const book = bookshelf.book(dracula)

        expect(book.lastPosition).toEqual(new Page(150))
    })
})
